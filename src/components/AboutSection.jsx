...
//data for the three "what makes us different" cards -writtn once here, 
//looped into markup 
const values = [
    {
        icon: "🌍",
...
export default function AboutSection() {
    return (
        <div className="about-content">
            {/* Header - rising delays fade the peices in one by one */}
            ...
            <div className="about-story-grid">
                ...
                <p className="about-body lead--light">
                    The Beans Place started in 2012 with one simple belief: greta coffee should be accessible to everyone-not just aficionados. We work directly with farming coopertives across four continets, paying above fair-tade prices to ensure qaulity from soil to sip.
                </p>
                <p className="about-body lead--light">
                    Every week, our roastmaster cups dozens of samples to select only the lots that meet our standard. Wether you prefer a bright, fruit Ethiopian Yirgacheffe or a deep, choclatey Colombian Supremo, we roast each batch to order so it arrives at peak freshness.
                </p>
                </ScrollReveal>

                {/* Photos slide in from the right, just after the text */}
                <ScrollReveal animation="fadeRight" delay={.2} className="about-story-images">
                    {/* Two images stacked so the second overlaps a corner */}
                    <div className="about-image-stack">
                        {/* alt text describes the image fro screen readers */}
                        <img
                        src={coffeePlants}
                        alt="Coffee plants in a farm"
                ...
                <StaggerContainer staggerDelay={0.15} className="about-cards-grid">
                    {values.map((v) => (
                        ...
                        <StaggerItem key={v.title} animation="fadeUp">
                            ...
                            <Card glass className="about-card-icon">
                            <CardHeader>
